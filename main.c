...
void processCommand(const char *cmd) {
  int tmpx, tmpy, btn, local, length;
  float tmpInterval;
  char alt, shift, ctrl, meta;
  char tmpkc;
  ...
  } else if (IS_CMD(cmd, "mouseScrollX")) {

    print_msg("Scrolling mouse.");
    sscanf(cmd, "mouseScrollX %d", &length);
    mouseScrollX(length);

  } else if (IS_CMD(cmd, "mouseScrollY")) {

    print_msg("Scrolling mouse.");
    sscanf(cmd, "mouseScrollY %d", &length);
    mouseScrollY(length);
  ...
  } else if(IS_CMD(cmd, "unicode")) {

    print_msg("Unicode");
    sscanf(cmd, "unicode %d", &local);
    unicodeKeyPress(local);
}
  else {
...
{
  int scrollDirection = -1, cnt;// 1 down -1 up
  printf("length: %d \n", length);
...
void unicodeKeyPress(long key)
{
  ...
  UniChar oneChar = (UniChar)keysym2ucs(key);
